...
状态（2026-09-01）：进行中。冻结 test 的 A～E 纯检索和 A/C/E 正式端到端
生成均已完成；正式 150 次调用无错误，自动引用指标和 30 题人工复核包已生成。
E 通过检索效果门禁，但失败 CPU 延迟、引用格式和有效引用覆盖门禁，因此默认
Dense 不切换。当前只等待人工完成 30 题蕴含标签与回答评分。
